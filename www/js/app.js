...
document.addEventListener("pageinit", function(e) {
	...
	switch (e.target.id)
	{
	   ...
	   case "page-merchantmenu":	
	   		// alert("asdf");   
	      $(".tab-active-page").html( getTrans("Merchant Menu","merchant menu") );	 
	      // $("#display-div").val('booking-pending');
			...
			switch (action)
			{
				...
				case "getSettings":      			       
			       ...
			       if (data.details.merchant_disabled_ordering=="yes"){
			       	   ...
			       	   $(".disabled_message_sel").show();
			       	   $(".disabled_message_text").show();
			       } else {
			       	   merchant_disabled_ordering.setChecked(false);
			       	   $(".disabled_message_sel").hide();
			       	   $(".disabled_message_text").hide();
			       }

			       $(".merchant_disabled_ordring_msg").val(data.details.merchant_disabled_ordring_msg);
			       $("#merchant_disabled_ordring_msg").val(data.details.merchant_disabled_ordring_msg);
			    ...
			    case "CategoryMenu":
			    	merchant_menu_list(data.details,'merchantmenu-list');
			    break;
...
function merchant_menu_list(data,div_id)
{
	var icon_trans_type=''; var icons=''; var icons2='';
	var htm='';
	$.each( data, function( key, val ) {        		  		  
		...
		htm+='<ons-col >';
		   htm+='<p class="margin2 small-font-dim small-font-dim-smaller concat-text bold">'+val.category_name+'</p>';
		   // htm+='<p class="margin2 small-font-dim small-font-dim-smaller">'+ getTrans("Order No",'order_no') +'. <b>'+val.order_id+'</b></p>';
		   // htm+='<p class="margin2 small-font-dim small-font-dim-smaller">'+val.delivery_date+'</p>';
		   htm+='<p class="status margin2 '+ val.status +' ">';
		   //htm+='<ons-icon icon="'+icons+'" class="icon '+icons2+'"></ons-icon>';
		   htm+=val.status;
		   htm+='</p>';
		 htm+='</ons-col>';
		 ...
		 htm+='<ons-col width="25%" class="text-right" >';
		   htm+='<ons-switch modifier="list-item" var="enabled_push" name="enabled_push" id="enabled_push"></ons-switch>';
		   htm+='<p class="small-font-dim orange-text">';
...
function displayOrderDetails(data)
{
	...
	var html='<ons-list-header class="header">';
     ...
     if ( data.time_diff=="yes"){
     html+='<ons-list-header class="header" style="text-align:center; background: white !important;">';
        html+='<ons-row>';
        html+='<ons-col>'+ getTrans("****ORDER AHEAD****",'order_details') +'</ons-col>';        
        html+='</ons-row>';
     html+='</ons-list-header>';
 	}
...
function statusList(data)
{
 ...
 //             htm+='<ons-col class="quantity-center trn" data-trn-key="quantity">Minutes for food preparation</ons-col>';
 //             htm+='<ons-col>';
 //             htm+='<input name="qty" type="number" class="qty numeric_only text-input text-input--transparent" value="10" style="text-align: center;">';
 //             htm+='</ons-col>';
 //         htm+='</ons-row>';
	// htm+='</ons-list>';	

	htm+='<ons-row class="row cart-total-wrap" style="margin-top: 6px; margin-bottom: 14px;">';
      htm+='<ons-col class="concat-text" width="20%">';
        htm+='<button class="button button--quiet" onclick="increase_prep(1)">';
      ...
      htm+='<ons-col class="" width="60%">';
         ...
         htm+='<ons-row class="row">';
             htm+='<ons-col class="quantity-center trn" data-trn-key="quantity">Minutes for food preparation</ons-col>';
             htm+='<ons-col>';
             htm+='<input name="prep_time" type="number" min="5" max="90" class="prep_time numeric_only text-input text-input--transparent" value="5" style="text-align: center; width: 78px;">';
             htm+='</ons-col>';
         htm+='</ons-row>';
         
      htm+='</ons-col>';
      htm+='<ons-col class="concat-text" width="20%">';
         htm+='<button class="button button--quiet" onclick="increase_prep(2)">';
...
function increase_prep(val_prep)
{
	if(val_prep==1)
	{
		pre_time=$(".prep_time").val();
		$(".prep_time").val(pre_time-5);
	}
	else
	{
		pre_time=$(".prep_time").val();
		added_val = parseFloat(pre_time) + parseFloat(5);
		$(".prep_time").val(added_val);
	}
}
...
function merchantmenu(done)
{
	...
	var action= "CategoryMenu";
	var div_id= 'merchantmenu-list';
...
{
	if(merchant_disabled_ordering.isChecked())
	{
		$(".disabled_message_sel").show();
		$(".disabled_message_text").show();
	}
 ...
 function select_msg()
 {
 	if($(".merchant_disabled_ordring_msg_sel").val()!='')
	{
		$(".merchant_disabled_ordring_msg").attr("disabled", "disabled"); 
		$(".merchant_disabled_ordring_msg").val('');
	}
	else
	{
		$(".merchant_disabled_ordring_msg").removeAttr("disabled");
	}
 }
...
function change_msg()
{
	if($(".merchant_disabled_ordring_msg").val()!='')
	{
		$(".merchant_disabled_ordring_msg_sel").attr("disabled", "disabled"); 
	}
	else
	{
		$(".merchant_disabled_ordring_msg_sel").removeAttr("disabled");
	}
}
